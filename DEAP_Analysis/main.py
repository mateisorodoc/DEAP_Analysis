...
DEAP_DATA_DIR = "F:/DEAP/data_preprocessed_python"  # Change this path to your DEAP dataset location

# Ensure the cupy cache directory exists
os.makedirs("cupy_cache", exist_ok=True)
os.environ["CUPY_CACHE_DIR"] = os.path.abspath("cupy_cache")
...
def gpu_psd_array_welch(data, sfreq, fmin, fmax, n_fft=512):
    """Calculate total power in frequency band using Welch's method on GPU."""
    psds = []
    for ch_data in data:
        ...
        total_power = cp.sum(pxx[mask]) * delta_f
        psds.append(total_power)
    return cp.array(psds), f[mask]
...
def process_and_save_psd(subject_idx, trial_idx):
    ...
    trial_data = cp.array(deap_eeg_data[trial_idx])  # Transfer trial data to GPU
    psd_results = {}

    for band, (fmin, fmax) in frequency_bands.items():
        psd, _ = gpu_psd_array_welch(trial_data, sfreq=sampling_frequency, fmin=fmin, fmax=fmax)
        psd_results[band] = psd.get()  # Transfer results back to CPU
